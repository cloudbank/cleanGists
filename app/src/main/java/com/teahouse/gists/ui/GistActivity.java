...
public class GistActivity extends DaggerAppCompatActivity {
  ...
  private Observer listObserver = new Observer<Resource<List<Gist>>>() {
    ...
    public void onChanged(@Nullable Resource<List<Gist>> listResource) {
      ...
      Log.i("OnChanged", "---->onChanged called for list");
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      ...
      Log.i(TAG, "oncreate reclaim savedinstancestate" + lastFetched);
  ...
  boolean isStale(String caller) {
    Log.i(TAG, "isStale " + caller + " " + lastFetched + " " + (System.currentTimeMillis() - lastFetched) + " " + (System.currentTimeMillis() - lastFetched > TimeUnit.MINUTES.toMillis(10)));
